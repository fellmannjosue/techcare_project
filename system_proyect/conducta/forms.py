...
class ReporteConductualForm(forms.Form):
    alumno = forms.ChoiceField(label="Estudiante")
    grado = forms.CharField(label="Grado", required=False, widget=forms.TextInput(attrs={'readonly': True}))
    materia = forms.ChoiceField(label="Materia")
    ...
    inciso_leve = forms.ModelChoiceField(
        label="Inciso Leve", queryset=IncisoConductual.objects.filter(tipo='leve', activo=True), required=False
    )
    grave = forms.BooleanField(label="Grave", required=False)
    inciso_grave = forms.ModelChoiceField(
        label="Inciso Grave", queryset=IncisoConductual.objects.filter(tipo='grave', activo=True), required=False
    )
    muy_grave = forms.BooleanField(label="Muy grave", required=False)
    inciso_muy_grave = forms.ModelChoiceField(
        label="Inciso Muy Grave", queryset=IncisoConductual.objects.filter(tipo='muy_grave', activo=True), required=False
    )
...
class ProgressReportForm(forms.Form):
    alumno = forms.ChoiceField(label="Estudiante")
    grado = forms.CharField(label="Grado", required=False, widget=forms.TextInput(attrs={'readonly': True}))
    semana_inicio = forms.DateField(label="Semana inicio", widget=forms.DateInput(attrs={'type': 'date'}))
    semana_fin = forms.DateField(label="Semana fin", widget=forms.DateInput(attrs={'type': 'date'}))
    comentario_general = forms.CharField(widget=forms.Textarea, required=False, label="Comentario General")
